...
int8_t stream_sensor_data_forced_mode(struct bme280_dev *dev)
{
    ...
    float vec_temp[10], vec_hu[10], vec_pres[10];
    float temp, press, hum;
    int count = 0;
    FILE * f;
    f = fopen("data.csv", "w+");
    fprintf(f, "data,temperatura,umidade,pressao\n");
    fclose(f);
	...
	{
		count = 0;
		float result_t=0, result_h=0, result_p=0;
		for(int i=0; i<10; i++){
			result_t += vec_temp[i];
			result_h += vec_hu[i];
			result_p += vec_pres[i];
		...
		printf("Salvando resultado %s %.2f %.2f %.2f\n", s, result_t, result_h, result_p);
		f = fopen("data.csv", "a");
